...
function parseBlock(raw: string, pattern: RegExp): { inner: string; stripped: string } {
...
function parseHints(block: string): HintItem[] {
  ...
  return block
    ...
    .map(line => {
      ...
      const tag = tagMatch ? (TAG_MAP[tagMatch[1].toLowerCase()] ?? 'action') : 'action'
      const withoutTag = line.replace(/^\[\w+\]\s*/, '')
      const [label, prompt] = withoutTag.split('|').map(s => s.trim())
      return { text: label ?? withoutTag, tag, prompt: prompt ?? label ?? withoutTag }
    })
...
function parseStructured(raw: string): ParsedDMResponse | null {
  ...
  const { inner: statusBlock, stripped: afterStatus } = parseBlock(raw, TAG_PATTERNS.status)
  const { inner: hintsBlock,  stripped: narrative    } = parseBlock(afterStatus, TAG_PATTERNS.hints)
...
function extractFreeformStatus(raw: string): { statusItems: string[]; cleaned: string } {
  const items: string[] = []
  let cleaned = raw
...
export function parseDMResponsePartial(raw: string): ParsedDMResponse {
  const firstTag = Math.min(
    ...[raw.indexOf('[STATUS]'), raw.indexOf('[HINTS]')]
